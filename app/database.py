...
def get_sync_status():
    with _get_engine().begin() as conn:
        ...
        pending_count = _sync_state_int(conn, "pending_change_count")
        pending = bool(last_dirty and (not last_push or str(last_dirty) >= str(last_push)))
        record_count = 0
...
def update_user(user_id, email, password=None, role="cashier", username=None):
    email = _normalize_email(email)
    username = (username or email if email else "").strip()
    if not email:
        raise AppError("Email kiriting.")
